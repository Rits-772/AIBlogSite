    ...
    <div className={`relative w-full ${className}`}>
      <div
        ...
        className={`max-h-[600px] overflow-y-auto p-4 ${
          displayScrollbar
            ? '[&::-webkit-scrollbar]:w-[8px] [&::-webkit-scrollbar-track]:bg-background [&::-webkit-scrollbar-thumb]:bg-border [&::-webkit-scrollbar-thumb]:rounded-[4px]'
            : 'scrollbar-hide'
        }`}
      ...
      >
        {items.map((item, index) => (
          ...
          >
            <div className={`p-4 bg-card rounded-lg border border-border transition-colors duration-200 ${selectedIndex === index ? 'bg-accent' : ''} ${itemClassName}`}>
              {typeof item === 'string' ? <p className="text-foreground m-0">{item}</p> : item}
            </div>
          </AnimatedItem>
        ))}
      </div>
